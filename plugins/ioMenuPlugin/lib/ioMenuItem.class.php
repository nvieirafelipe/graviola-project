...
class ioMenuItem implements ArrayAccess, Countable, IteratorAggregate
{
  ...
  public function render($depth = null, $renderAsChild = false)
  {
    ...
    else
    {
      ...
      {
        // if there is no user logged the menu must be smaller to fit the authentication on the header
        if (sfContext::getInstance()->getUser()->isAuthenticated())
          $attributes['class'] = 'column span-14 main_menu';
        else
          $attributes['class'] = 'column span-8 main_menu';
      }
    }
